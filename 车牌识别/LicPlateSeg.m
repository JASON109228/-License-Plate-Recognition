function character_image = LicPlateSeg(plate_im)
...
bw = im2bw(I); % 二值化
[a, b] = size(bw);
Ibw = ones(size(bw)) - bw; %黑底白字
figure(7), imshow(Ibw),title('黑底白字的车牌图片');
%% 裁车牌上下多余边缘
...
for i = 1 : length(c1) - 1
    ...
    for j = 1:number
        [rj, ~] = find(L == j);
        if max(rj) - min(rj) > 0.42 * a % 用高度判断该连通区是否是字符
            h(1,index) = min(rj);
            h(2,index) = max(rj);
            index = index + 1;
        end
...
figure(8), imshow(I1),title('裁掉车牌上下多余边缘后的图片');
...
if numtemp ~= 0
    for iL = 1:numtemp
        ...
        btemp1 = min(ctemp);
        btemp2 = max(ctemp);
        if atemp2 - atemp1 > 0.95*size(I2,1)
